...
dotProdMatrix <- function(allSpectra = NULL, spectraNames = NULL, 
binSizeMS2 = NULL) {
    # error handling
    ...
    labelsTmp <- paste0("(", seq(binSizeMS2, (maxMass - binSizeMS2), 
    binSizeMS2), 
    ",", seq((2 * binSizeMS2), maxMass, binSizeMS2), "]")
    massBinsIndivTmp <- cut(allSpectra[, 1], breaks = seq(binSizeMS2, maxMass,
    binSizeMS2), 
    labels = labelsTmp)
    # empty bins
    ...
    allBinNames <- paste0(rep(unique(spectraNames), each = length(labelsTmp)), 
    rep(labelsTmp, 
