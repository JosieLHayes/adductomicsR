    ...
    if (nrow(spectrum.df) > minPeaks) {
        ...
        if (minIntIndx < (nrow(spectrum.df) - 1)) {
            for (k in minIntIndx:(nrow(spectrum.df) - 1)) {
                ...
                fit <- coef(RcppEigen::fastLm(as.numeric(spectrum.df[seq_len(k),
                2])~ 
                peakIndx[seq_len(k)]))
        ...
        DNF.tmp <- list(Above.noise = spectrum.df, metaData = 
        data.frame(Noise.level = Noise.level, 
        IntCompSpec = IntCompSpec, TotalIntSNR = TotalIntSNR, 
        nPeaks = nrow(spectrum.df), 
        stringsAsFactors = FALSE), aboveMinPeaks = aboveMinPeaks)    
    } else {
        DNF.tmp <- list(Above.noise = spectrum.df, metaData = 
        data.frame(Noise.level = 0, 
        IntCompSpec = 0, TotalIntSNR = 0, nPeaks = nrow(spectrum.df), 
