...
#' @return list
peakIdQuant_newMethod <- function(mzTmp = NULL, rtTmp = NULL, 
peakRangeRtSub = NULL, rtDevModel = NULL, isoPat = NULL,
isoPatPred = NULL, minSimScore = 0.96, maxPpm = 4, 
gaussAlpha = 16, spikeScans = 2, minPeakHeight = 5000, 
maxRtDrift = 20, showPlots = FALSE, 
isoWindow = 10, maxGapMs1Scan = 5, intMaxPeak = FALSE) {
    ...
    maxMz & peakRangeRtSub[, 
    1] > minMz, , drop = FALSE]
    if (nrow(peakRangeMIM) > 0) 
        {
            # sum signal rt
            peakRangeMIM <- peakRangeSum(peakRangeMIM, spikeScans, 
            rtDevModel, gaussAlpha = gaussAlpha, 
        maxEmptyRt = maxGapMs1Scan)
            ...
            peaksMIM <- peaksMIM[peakRangeMIM[peaksMIM, 2] >= minPeakHeight]
            ...
            if (length(peaksMIM) > 0) 
                {
                    ...
                    truePeaks <- truePeakTrough(peaksMIM, troughsMIM,
                    peakRangeMIM)
                    ...
                    if (length(truePeaks) > 0 & length(trueTroughs) > 0) 
                        {
                            ...
                            peaksMIM <- nAdjPeaks(peaksMIM, troughsMIM,    
                            peakRangeMIM)
                            ...
                            if (length(truePeaks) > 0 & length(trueTroughs)>0) 
                                {
                                    ...
                                    if (showPlots == TRUE) {
                                        ...
                                        2], col = "blue")
                                        abline(v = c(peakRangeMIM[peaksMIM, 3]-
                                        isoWindow, 
                                        peakRangeMIM[peaksMIM, 
                                        3] + isoWindow), 
                                        lty = (seq_len(length(peaksMIM))) + 1,
                                        lwd = 0.1)
                                        abline(v = peakRangeMIM[peaksMIM, 3], 
                                        lty = (seq_len(length(peaksMIM))) + 
                                        1, lwd = 0.1)
                                        abline(v = c(rtTmp - maxRtDrift, 
                                        rtTmp + maxRtDrift), 
                                    lty = 2, col = "red")
                                    }
                                    ...
                                    peakRangeMIM[peaksMIM
                                    , , drop = FALSE])
                                    colnames(peaksTableTmp) <- c("isoPat",
                                    "mass", "intensity", 
                                    "predRtLoess", "RT", "seqNum")
                                    # dist iso table
                                    distIsoTable <- matrix(nrow = 0, 
                                    ncol = ncol(peaksTableTmp))
                                    colnames(distIsoTable) <- c("isoPat",
                                    "mass", "intensity", 
                                    "predRtLoess", "RT", "seqNum")
                                    # 2. id isotope peaks (first 5)
                                    for (isoIt in 2:6) {
                                        peakRangeTmp <-peakRangeRtSub[
                                        peakRangeRtSub[, 1] <
                                        (maxMz + isoPat[isoIt]) & 
                                        peakRangeRtSub[, 1] > 
                                        ...
                                        if (nrow(peakRangeTmp) > 0) {
                                            ...
                                            spikeScans, 
                                            rtDevModel)  
                                            peaksTmp <- findPeaks(peakRangeTmp[
                                            , 2], m = 1)
                                            ...
                                            if (!is.null(peaksTmp)) {
                                                ...
                                                peakRangeTmp <- cbind(names(
                                                isoPat)[isoIt], peakRangeTmp)
                                                colnames(peakRangeTmp) <- c(
                                                "isoPat", "mass",
                                                "intensity", 
                                                "predRtLoess", "RT", "seqNum")
                                                # rbind dist iso info
                                    ...
                                    4]), method = "median", members = NULL)
                                        # cut tree according to absolute rt
                                        #difference of less than isoWindow
                                        #seconds
                                        rtGroups <- cutree(hr, h = isoWindow)
                                        nIsoDetected <- table(rtGroups[
                                        duplicated(paste0(peaksTableTmp[, 
                                        1], "_", rtGroups)) == FALSE])
                                        nIsoDetected <- nIsoDetected[names(
                                        nIsoDetected)
                                        %in% rtGroups[seq_len(length(
                                        peaksMIM))]]
                                        # at least 3 peaks detected
                                        nIsoDetected <- nIsoDetected[
                                        nIsoDetected >= 3]
                                        if (length(nIsoDetected) > 0)  {
                                            # subset peaks table
                                            indxTmp <- rtGroups %in% 
                                            as.numeric(names(nIsoDetected))
                                            rtGroups <- rtGroups[indxTmp]
                                            peaksTableTmp <- peaksTableTmp[
                                            indxTmp, , 
                                            drop = FALSE]
                                            ...
                                            # empty
                                            # matrix for results
                                            dotProdRes <- matrix(0, ncol = 9, 
                                            nrow = length(unique(rtGroups)))
                                            dotProdRes[, 1] <- unique(rtGroups)
                                            # loop through diff rt groups and
                                            # calc dot prod
                                            for (rtGr in seq_len(nrow(
                                                dotProdRes))) {
                                                ...
                                                # isotope
                                                splitF <- as.factor(
                                                peaksTableTmp[, 1])[indxTmp]
                                                levels(splitF) <- paste0(
                                                "iso", 0:5)
                                                indxTmp <- split(indxTmp, 
                                                splitF)
                                                # replace integer(0) with zero
                                                indxTmp[vapply(indxTmp, length, 
                                                FUN.VALUE=numeric(1)) == 0]<-0
                                                # all combination of different
                                                # MIM and isotope 
                                                #peaks
                                                allComb <- do.call(expand.grid,
                                                indxTmp)
                                                # calculate dot prods for all
                                                # combinations
                                                dotProds <- t(apply(allComb, 1,
                                                function(indxComb) 
                                                {
                                                    nonZeroTmp <- which(
                                                    indxComb != 0)
                                                    isoPattPercent <- rep(0, 6)
                                                    # subset intensity values
                                                    isoPattInt <- as.numeric(
                                                    peaksTableTmp[
                                                    as.numeric(indxComb[
                                                    nonZeroTmp]), 3])
                                                    ...
                                                    isoPattPercent * 
                                                    0.1)[seq_len(3)]) | all(
                                                    sign(diff(isoPattPercent))[
                                                    seq_len(2)] == 
                                                    isoPatPred$sign[2:3])) * 1
                                                    dotProd <- c(corrPattTmp,
                                                    dotProd)
                                                }))
                                                    # concatenate results add
                                                    #to table
                                                    dotProdRes[rtGr, 2:9] <- 
                                                    c(ifelse(any(dotProds[, 
                                                    1] == 1), 1, max(
                                                    dotProds[, 2])), 
                                                    max(dotProds[, 2]), 
                                                    as.numeric(allComb[
                                                ...
                                                stringsAsFactors = FALSE)
                                                    ...
                                                    "AdjRetentionTime"]
                                                    # integrate peak
                                                    peakIntRes <-peakIntegrate(
                                                    peakRangeMIM, peakStart, 
                                                peakEnd, mzTmp, rtTmp)
                                                    peakRangeMIM <- 
                        ...
                        if (length(resList) == 0) {
                            ...
                            peakRangeMIM <- data.frame(peakRangeMIM, 
                            stringsAsFactors = FALSE)
                            colnames(peakRangeMIM) <- c("mass", "intensity", 
                            "AdjRetentionTime", "retentionTime","seqNum")
                            peakRangeMIM$peaks <- ""
                            peakRangeMIM$troughs <- ""
                            peakRangeMIM <- peakRangeMIM[, c("mass", 
                            "intensity",
                            "AdjRetentionTime",  "retentionTime", "seqNum", 
                            "peaks", "troughs")]
                            resList <- list(peakRangeMIM = peakRangeMIM)
                        }
                        return(resList)
                    }  # end function